...
**解压到指定目录后多出一层目录**：打包前端构建产物 `dist` 部署时最常遇到——期望解压后文件直接落在目标目录里，结果里面多包了一层 `dist/`：

```bash
tar -czf dist.tar.gz dist
#     问题出在打包这一步：参数写的是 dist 这个目录名，目录本身被打了进去
#     解压时自然就会先冒出一层 dist/，和上一个坑同一个根源——打包时写什么路径，解压就还原什么路径

tar -xzf dist.tar.gz -C /opt/deploy --strip-components=1
#     解法一（包已经打好了）：--strip-components=1 解压时剥掉路径的第一层
#     dist/index.html 就变成直接解出 index.html

tar -czf dist.tar.gz -C dist .
#     解法二（重新打包）：-C dist 先切进目录，再用 . 打包"目录里的内容"
#     这样打出来的包解压后没有 dist 层，给谁解压都干净
```

**解压完用 mv 挪文件，子目录里的文件没跟着过去**：接着上面的部署场景——解压到了临时目录，想用 `mv` 把所有内容挪进目标目录，结果发现子目录下的文件没动：

```bash
mv /tmp/extracted/* /opt/deploy/
#     如果 /opt/deploy/ 里已经存在同名的非空子目录（比如两边都有 assets/），
#     mv 会报 "Directory not empty" 拒绝移动那个子目录——mv 只会整体重命名目录，
#     不会把两个同名目录的内容合并到一起；普通文件正常移过去了，
#     撞名的子目录和它下面的文件全部留在原地，很容易只看见部分成功就以为全挪完了

rsync -a /tmp/extracted/ /opt/deploy/ && rm -rf /tmp/extracted
#     要合并目录内容，用 rsync 才是对的工具——mv 的本质是"重命名整个目录"，
#     撞名就无解；rsync 是递归复制，同名子目录会钻进去逐个合并文件
#     rsync 不只用于远程传输，两边都写本地路径时就是一个加强版的 cp
#     源路径末尾的 / 表示"同步目录里的内容"，这个语义在 rsync 那篇讲过
```

更省事的做法是回到上一个坑的解法：解压时直接 `-C` 到最终目录并配合 `--strip-components`，文件一步到位，根本不需要 `mv` 二次搬运。
...
| `-C DIR` | 解压到指定目录；打包时则表示"先切进这个目录再打包" |
| `--strip-components=N` | 解压时剥掉路径开头 N 层目录，治"解压多一层" |
...
需要在机器之间同步目录、增量传输大文件时，`tar` 打包再拷贝就不是最优解了——这类场景见 [rsync 增量同步实战：常用参数、排除规则与远程同步](/rsync增量同步实战-常用参数与远程同步)，上面 mv 合并目录那个坑的正解也在里面。
