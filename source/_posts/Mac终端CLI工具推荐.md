...
> 详细安装与配置见：[终端搜索三件套：fzf + ripgrep + fd + jq](/终端搜索三件套-fzf-ripgrep-fd)
...
> 详细安装与配置见：[Shell 历史与目录跳转的现代替代：atuin + zoxide](/atuin-zoxide-终端历史与目录跳转)
...
> 详细安装与配置见：[终端启动界面：fastfetch + pokemon-colorscripts](/fastfetch-pokemon-colorscripts-终端外观)
...
> ripgrep、fd、jq 的详细用法见：[终端搜索三件套：fzf + ripgrep + fd + jq](/终端搜索三件套-fzf-ripgrep-fd)
...
> 详细安装与配置见：[文件查看升级：bat + eza + glow + mdcat](/文件查看工具-bat-eza-glow-mdcat)
...
> 详细安装与配置见：[yazi：终端里的三栏文件管理器](/yazi终端文件管理器)
...
> 详细安装与配置见：[终端里的 Git：git-delta + lazygit + gh](/git-delta-lazygit-gh-终端Git工具链)
...
> 详细安装与配置见：[从零开始用 Neovim：LazyVim 让配置不再是障碍](/neovim-lazyvim-终端编辑器)
