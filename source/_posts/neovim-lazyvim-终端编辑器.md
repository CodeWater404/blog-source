...
Space + g + l   打开 Git Log：左边提交列表，右边是选中提交的改动预览
Space + g + L   同上，但范围是当前目录，不是整个仓库
```

### Git Log 预览卡顿：翻到某些提交时右侧空白很久

`Space g l` 打开的 Git Log，绝大多数提交上下翻都很流畅，但翻到个别提交时，右侧预览会空白好一会儿才出内容。

问题不在 Neovim，在这些提交本身。先看卡的那个提交改了什么：

```bash
git show --stat=200 --format= <commit>
#         --stat=200：统计行总宽度放宽到 200，文件路径不被截断
#         --format=：不输出提交说明，只留改动文件的统计
```

如果里面有前端构建产物（比如 `web/dist/assets/index-xxxx.js`），就是原因。压缩后的 JS 整个文件挤在少数几行里，单行能有几百 KB；文件名还带内容哈希，每次重新构建都换名，git 看到的就是"删掉整个旧文件、新增整个新文件"。一个提交的 diff 因此有 2~3MB，其中真正的代码改动可能只有十几 KB。

预览用的是 LazyVim 自带的 snacks 选择器，默认的 `fancy` 风格会拿到 `git show` 的全部输出（没有大小上限），再逐行加边框、行号和语法高亮，预览窗口默认还开着自动折行。几百 KB 的超长单行，对这套处理方式是很重的负担。

解决办法是让 git 不再展开这些文件的 diff，在仓库的 `.git/info/attributes` 里加一行：
...
- 这个文件在 `.git/` 目录里，不属于任何提交，不会被推送，同事的仓库不受影响，仓库里存的内容也一个字节不变。
- 只有匹配的文件被折叠，同一个提交里的其他文件（真正的代码）照常完整显示。
- 想撤销，删掉那一行即可。
- 同样的写法适用于其他没人会去读的生成文件，比如 `dist/**`、`*.min.js`，或者测试生成的巨型数据文件。

实测一个 2824KB 的提交，加上这一行后 `git show` 的输出降到 12KB，Git Log 预览恢复流畅。
